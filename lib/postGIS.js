...
exports.pgCreateRelated = async function(res, pool, select, scale, dir, czmlfile, callback) {
   ...
   try {
      ...
      for (var i=0, tot=rows.length; i<tot; i++) {
         ...
         czmlfile = czml.createPoint(rows[i].id, rows[i].website, description, icon, scale, Number(rows[i].x_coord), Number(rows[i].y_coord), Number(rows[i].z_coord), czmlfile);
      }
   } finally {
      client.release()
      callback(null, res, czmlfile);
