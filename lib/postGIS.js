...
export async function pgCreatePoint(pool, schema, table, filterField, type, image) {
  ...
  try {
    ...
    const result =
      escapedFilter && type ? await client.query(sql, [type]) : await client.query(sql);
...
export async function pgFindGisPOI(pool, group, distance, color, type = '%') {
  ...
  const typeCondition =
    type === '%' ? `AND (g."type" LIKE $3 OR g."type" IS NULL)` : `AND g."type" = $3`;
...
export async function pgGetMultiline(pool, maintable, group, linewidth, color, type = '%') {
  ...
  const escapedTable = client.escapeIdentifier(maintable);
  const typeCondition = type === '%' ? `AND ("type" LIKE $2 OR "type" IS NULL)` : `AND "type" = $2`;

  const select = `SELECT ST_AsText(geom) AS wkt_geom, id, name, description FROM ${escapedTable} WHERE "group" = $1 ${typeCondition}`;
...
export async function pgExportMultiline(kmlfile, pool, maintable, group, type) {
  ...
  const escapedTable = client.escapeIdentifier(maintable);
  const typeQuery =
    type === '%' || !type ? 'AND ("type" LIKE $2 OR "type" IS NULL)' : 'AND "type" = $2';
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const placemark = kml.createLine(row.name, featureDescription, group, coords);
...
export async function pgCreateLine(pool, czmlfile, maintable, group, linewidth, color) {
  const client = await pool.connect();
