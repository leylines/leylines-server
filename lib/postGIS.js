...
export async function pgCreatePoint(pool, schema, table, filterField, type, image) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    czml.createPoint(newPackets, row, featureDescription, scale, finalIcon, pixelOffset);
  }
...
export async function pgCreateLine(czmlfile, maintable, group, linewidth, color) {
  ...
  const select = "SELECT ST_AsGeoJSON(geom), name, description FROM " + maintable + " WHERE \"group\" = '" + group + "';";
  ...
  try {
    ...
    for (const row of rows) {
      ...
      var multiline = [];
      for (let i=0, subtot=multilines.coordinates.length; i<subtot; i++) {
      ...
      czmlfile = czml.createCorridor(i, row.name, row.description, linewidth, color, multiline, czmlfile);
    }
  } finally {
    client.release()
    return(czmlfile);
...
export async function pgCreateArea(czmlfile, pool, maintable, name, color, extrudeheight) {
    // Wir holen uns die Koordinaten direkt als flaches Array via PostGIS, das spart massiv JS-Rechenzeit
    // ST_Dump sorgt dafür, dass auch MultiPolygone korrekt in Einzelpolygone zerlegt werden
    const query = {
    ...
    try {
        ...
        rows.forEach((row, index) => {
            ...
            czml.createPolygon(uniqueId, row.name, row.description, color, extrudeheight, polygonCoords, czmlfile);
        });
    } catch (err) {
