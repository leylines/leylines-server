...
export function createFeatureInfo(row, rowtype, imagetype) {
  ...
  const imagePath = row.image || (rowtype === "leylines" ? `/popup/${row.name.replace(/ /g, "_")}.jpg` : null);
  const imagealt = row.name;

  const wikipedialink = row.name && rowtype === "leylines" 
  ...
  const hasImage = row.image || (rowtype === "leylines" && utils.imageExistsInCache(imagePath));
