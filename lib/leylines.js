...
exports.createRamar = async function(req, res, pool) {
   ...
   var select = "SELECT id, site, description, object_type, x_coord, y_coord, country FROM ramar.poi";
   ...
   try {
      ...
      for (var i=0, tot=rows.length; i<tot; i++) {
         var description = "<!--HTML-->\r\n";
         description += "<table>";
         description += "<tr><td width=\"120px\">Description</td><td width=\"50px\"></td><td>" + rows[i].description + "</td></tr>";
         description += "<tr><td>Country</td><td></td><td>" + rows[i].country + "</td></tr>";
         description += "</table>";
         description += "\r\n</p>";
         var icon = "/images/megalithic/" + rows[i].object_type + ".png";
         var scale = 0.7;
