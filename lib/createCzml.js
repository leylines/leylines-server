...
export async function createMultiline(linewidth, color, startpoint, endpoints, results) {
  for (let i=0, tot=endpoints.length; i<tot; i++) {
    ...
    let polylineid = startpoint + "" + endpoints[i];
...
export async function createPolygonReference(referencesvalues) {
  const result = [];
  for (let j=0, tot=referencesvalues.length; j<tot; j++) {
    result.push(referencesvalues[j] + "#position");
  }
  return result;
};

export function createPolygon(id, name, description, colorvalue, extrudeheight, polygonPositions, results) {
  const packet = {
    id: id,
    name: name,
    description: description,
