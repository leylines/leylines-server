...
var leylinesHost = createVirtualHost("maps.leylines.ch", "prod");
var demoHost = createVirtualHost("demo.leylines.ch", "demo");
var testHost = createVirtualHost("test.leylines.ch", "test");
var wwwHost = createVirtualHost("w3.leylines.ch", "www");
...
function endpoint(path, router) {
    if (options.verbose) {
        console.log('http://localhost:' + options.port + '/api/v1' + path);
    }
...
=======
if (options.verbose) {
    console.log('Listening on these endpoints:');
}
